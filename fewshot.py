...
from shared.const import task_fewshot_ner_labels, get_labelmap
from entity.utils import convert_dataset_to_samples, batchify, NpEncoder, getlabel_embedding
from entity.models import EntityModel_label_RICON_fs  # rz+
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_length', type=int, default=25,
                        help="LST length")
    ...
    args.train_data = os.path.join(args.data_dir, 'train_250shot.json')
    ...
    ner_label2id, ner_id2label = get_labelmap(task_fewshot_ner_labels[args.task])

    num_ner_labels = len(task_fewshot_ner_labels[args.task]) + 1  # 取task_ner_labels列表中的ace05数据集标签数+1=8
    ...
    model = EntityModel_label_RICON_fs(args)
    ...
    dev_data = MyData(args.dev_data)  # 测试集直接当验证集
    ...
    if args.do_train:
        ...
        param_optimizer = list(model.bert_model.named_parameters())  # 这里把label_bert的参数也添加了，故需要去掉？？？
        # param_optimizer = []
        # for n, p in model.bert_model.named_parameters():
        #     if 'label_bert' in n:          # 去掉label_bert的参数，不更新label_bert
        #         continue
